...
<!--
    <SectionSlide title="Compiler Directive" />
    <TemplateDemoSlide :files="files" active="Directive" explorer="CompilerDirectiveExample" />
    <SectionSlide title="Compiler directive">
      <CodePreview language="js" @click.stop="() => {}">
        function consoleDirective(el, directive, warn) {
          if (!el.props) el.props = []
          const method = directive.arg || 'log'
          
          el.props.push({
            name: 'data-log', 
            value: `console.${method}(${directive.value})`
          })
        }
      </CodePreview>
    </SectionSlide>
    <SectionSlide title="Use compiler directive">
      <CodePreview language="js" @click.stop="() => {}">
        module.exports = {
          chainWebpack: config => {
            config.module
